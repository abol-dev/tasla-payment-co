...
const Card16Podcast: FC<Card16PodcastProps> = ({
  ...
  return (
    <div
      className={`nc-Card16Podcast relative flex flex-col group ${className}`}
    ...
    >
      ...
      {/* MAIN CONTENT */}
      <div className="w-11/12 -mt-32 transition-all transform group-hover:w-full group-hover:translate-y-5">
        <div
          className={`px-5 flex items-center space-x-4 h-14  ${
            !IS_AUDIO ? "relative opacity-0 z-[-1]" : ""
          }`}
        ></div>
        <div className="flex flex-col flex-grow p-5 mt-5 bg-white rounded-tl-none shadow-xl dark:bg-neutral-900 dark:shadow-2xl rounded-3xl ">
          <h2 className="block text-xl font-semibold nc-card-title text-neutral-900 dark:text-neutral-100 ">
            <Link to={href} className="line-clamp-1" title={title}>
              {title}
            </Link>
          </h2>
          <span className="block mt-3 mb-5 text-sm text-neutral-500 dark:text-neutral-400">
